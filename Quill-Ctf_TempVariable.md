...
function whitelistuser(address user) public onlyOwner {
    ...
    //@bug User can call this function again with same amount 
    function remove (
        address _from,
        uint256 _amount
    ) public isUserBlacklisted(_from)  {
        require(_from == msg.sender, "unauthorized");
        uint256 _accountbalance = _balances[_from]; //! Capturing the balance 
        ...
        if (_amount == _accountbalance) {
            ...
            _accountbalance -= _amount; //! Subtracts 
            _balances[_from] = _accountbalance;// ! Reassigns the same value again 
        }
